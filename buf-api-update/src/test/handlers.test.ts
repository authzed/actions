import { fileFormatHandlers } from "../handlers.js";
...
describe("shell script format handler", () => {
  ...
  it("properly changes the buf version", () => {
    const [contents, updated] = handler(
    ...
    expect(contents).toBe(
      `#!/usr/bin/env -S buf generate buf.build/authzed/api:somenewversion --template`,
    );
  });
  ...
  it("does not change a static buf version", () => {
    const [, updated] = handler(
      `#!/usr/bin/env -S buf generate buf.build/authzed/api:dc592e107033a7a4336935cf94fb90426719508d --template`,
      "dc592e107033a7a4336935cf94fb90426719508d",
    );
...
describe("buf-gen-yaml format handler", () => {
  it("properly changes the buf version", async () => {
    ...
    const [input, expected] = await Promise.all(
      ["./yaml/buf.gen.yaml", "./yaml/buf.gen.yaml.expected"].map(readFileAsString),
    );
